...
type
  ...
  TfMain = class(TForm)
    ...
    procedure bSortCoutryClick(Sender: TObject);
...
begin
 //если данных нет - выходим:
  ...
  if MessageDlg('Требуется подтверждение',
                'Вы действительно хотите удалить номер богажа "' +
                SG.Cells[0, SG.Row] + '"?',
...
begin
  ...
  if fEdit.ModalResult = mrOk then begin
    ...
    SG.Cells[3, SG.Row]:= fEdit.eCountry.Text;
  end;
end;

procedure TfMain.bImageClick(Sender: TObject);
begin
    fImage.ShowModal;
end;
...
procedure TfMain.bSortCoutryClick(Sender: TObject);
begin
  //если данных в сетке нет - просто выходим:
  if SG.RowCount = 1 then exit;
  //иначе сортируем список:
...
begin
  ...
  SG.Cells[0, 0]:= 'Имя';
  SG.Cells[1, 0]:= 'Номер';
  SG.Cells[2, 0]:= 'Аэропорт';
  SG.Cells[3, 0]:= 'Город назначения';
  SG.ColWidths[0]:= 365;
  SG.ColWidths[1]:= 150;
  SG.ColWidths[2]:= 150;
  SG.ColWidths[3]:= 150;
  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[3, SG.RowCount-1]:= MyCont.Country;
    end;
  finally
    CloseFile(f);
  end;
